...
SELECT customers.company_name, concat(employees.first_name, ' ', employees
    .last_name) as employee_full_name FROM orders
INNER JOIN
...
WHERE customers.city = 'London' AND employees.city = 'London' AND shippers
...
WHERE NOT EXISTS(SELECT customer_id FROM orders WHERE customers.customer_id
                                                          = orders.customer_id);
